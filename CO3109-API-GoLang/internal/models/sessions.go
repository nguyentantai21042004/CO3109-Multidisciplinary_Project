...
var SessionWhere = struct {
	...
	AccessToken:  whereHelperstring{field: "\"sessions\".\"access_token\""},
	...
	DeviceName:   whereHelpernull_String{field: "\"sessions\".\"device_name\""},
	...
	DeletedAt:    whereHelpernull_Time{field: "\"sessions\".\"deleted_at\""},
}
...
var (
	sessionAllColumns            = []string{"id", "user_id", "access_token", "refresh_token", "user_agent", "ip_address", "device_name", "expires_at", "created_at", "updated_at", "deleted_at"}
	sessionColumnsWithoutDefault = []string{"user_id", "access_token", "refresh_token", "expires_at"}
	sessionColumnsWithDefault    = []string{"id", "user_agent", "ip_address", "device_name", "created_at", "updated_at", "deleted_at"}
...
func Sessions(mods ...qm.QueryMod) sessionQuery {
	mods = append(mods, qm.From("\"sessions\""), qmhelper.WhereIsNull("\"sessions\".\"deleted_at\""))
	q := NewQuery(mods...)
...
func FindSession(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*Session, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"sessions\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *Session) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"sessions\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func (o SessionSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"sessions\" SET %s WHERE "+
...
func SessionExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"sessions\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
