...
// @Produce json
// @Success 200 {object} response.Resp "Success"
// @Failure 400 {object} response.Resp "Bad Request, Error errUserNotFound(110003)"
// @Failure 500 {object} response.Resp "Internal Server Error"
...
func (h handler) DetailMe(c *gin.Context) {
	...
	if err != nil {
		h.l.Warnf(ctx, "auth.http.DetailMe.processDetailMeRequest: %v", err)
		response.Error(c, h.mapErrorCode(err))
		return
	}
	...
	if err != nil {
		mapErr := h.mapErrorCode(err)
		if slices.Contains(NotFound, err) {
			h.l.Warnf(ctx, "auth.http.DetailMe.DetailMe.NotFound: %v", err)
		} else {
			h.l.Errorf(ctx, "auth.http.DetailMe.DetailMe: %v", err)
		}
		response.Error(c, mapErr)
		return
	}

	response.OK(c, h.newDetailMeResp(o))
}
...
// @Failure 500 {object} response.Resp "Internal Server Error"
// @Router /api/v1/auth/social-login/{provider} [POST]
...
// @Failure 400 {object} response.Resp "Bad Request, Error errWrongBody(110002)"
// @Failure 500 {object} response.Resp "Internal Server Error"
// @Router /api/v1/auth/social-callback/{provider} [GET]
