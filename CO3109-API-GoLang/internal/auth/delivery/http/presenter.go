...
func (h handler) newLoginResp(o auth.LoginOutput) *loginResp {
	return &loginResp{
		User: userObj{
			ID:    o.User.ID,
			Email: o.User.Email,
			Name:  o.User.FullName.String,
			Role:  respObj{ID: o.Role.ID, Name: o.Role.Name},
		},
...
// Detail Me API Request
type detailMeResp struct {
	ID       string  `json:"id"`
	FullName string  `json:"full_name"`
	Email    string  `json:"email"`
	Name     string  `json:"name,omitempty"`
...
func (h handler) newDetailMeResp(o auth.DetailMeOutput) detailMeResp {
	return detailMeResp{
		ID:       o.User.ID,
